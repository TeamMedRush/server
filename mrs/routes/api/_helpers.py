...
def auth_payload(request: Request, data: dict | None = None) -> dict:
  payload = {
    "token": request.meta.get("token") or request.meta.get("auth-token") or request.meta.get("user-token"),
  }
  ...
  payload["password"] = data.get("current_password", data.get("password"))

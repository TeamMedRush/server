...
from mrs.services.partners import get_inventory, set_inventory_items

@Router.endpoint("/api/v1/partner/account")
async def partner_account(request: Request) -> Response:
  ...
  if request.method == "POST":
    try:
      ...
      result = sign_up("partner", data["email"], data["password"], data)
  ...
  if request.method in ("PATCH", "PUT"):
    try:
      data = json_body(request)
      identity = resolve_credentials(auth_payload(request, data), "partner")
...
async def partner_inventory(request: Request) -> Response:
  ...
  if request.method == "PUT":
    try:
      ...
      inventory = set_inventory_items(identity["profile"]["id"], items)
    except ValueError as error:
      return response.json(400, {"error": str(error)})
    except LookupError as error:
      return response.json(404, {"error": str(error)})
